...
import io.github.italodgsilva.presentation.exception.GameGatewayConnectionTimeoutExceptionMapper
...
class GameGatewayConnectionTimeoutExceptionMapperTest {
    @Test
    fun `must map an exception to response`() {
        val exception = GameGatewayConnectionTimeoutException(gateway = "test")
        val response = GameGatewayConnectionTimeoutExceptionMapper().toResponse(exception)
